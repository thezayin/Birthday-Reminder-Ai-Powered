...
class InterstitialAdManagerImpl : InterstitialAdManager {
    ...
    private var adId: String = "ca-app-pub-2913057115284606/9261254217"
    ...
    override fun loadAd(activity: Activity) {
        ...
        if (interstitialAd == null) {
            // Create an ad request and load the rewarded interstitial ad
            val adRequest = AdRequest.Builder().build()

            // Request to load the rewarded interstitial ad
            RewardedInterstitialAd.load(
                activity, // The activity where the ad will be displayed
                adId,     // The ad unit ID for the rewarded interstitial ad
                adRequest, // The ad request to be sent to load the ad
                object : RewardedInterstitialAdLoadCallback() { // Callback for handling loading result
                    // Called when the ad is successfully loaded
                    override fun onAdLoaded(ad: RewardedInterstitialAd) {
                        Timber.tag("RewardedInterstitialAd").d("Ad loaded successfully")
                        interstitialAd = ad // Save the loaded ad
                    }

                    // Called if the ad fails to load
                    override fun onAdFailedToLoad(adError: LoadAdError) {
                        Timber.tag("RewardedInterstitialAd").d("Ad failed to load: ${adError.message}")
        ...
        if (showAd) {
            ...
            interstitialAd?.let { ad -> // If the ad is loaded, show it
                ad.show(activity) { rewardItem: RewardItem ->
                    // Callback when the user gets rewarded
                    Timber.tag("RewardedInterstitialAd").d("User earned reward: ${rewardItem.type}, ${rewardItem.amount}")
                ...
                ad.fullScreenContentCallback = object : FullScreenContentCallback() {
                    ...
                    override fun onAdFailedToShowFullScreenContent(adError: AdError) {
                        Timber.tag("RewardedInterstitialAd").d("Ad failed to show: ${adError.message}")
                    ...
                    override fun onAdDismissedFullScreenContent() {
                        Timber.tag("RewardedInterstitialAd").d("Ad dismissed. Reloading the ad.")
